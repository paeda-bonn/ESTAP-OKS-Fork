...
final class TimeSlot
{
    ...
    {
        $sql = "INSERT INTO time_slots (start_time, end_time, date, teacher) VALUES (:start_time, :end_time, :date, 'ALL')";
    ...
    public static function createTeacher($startTime, $endTime, $teacherId, $clean, $date)
    {
        ...
        if($clean){
            $sql= "DELETE FROM `time_slots` WHERE `teacher`='$teacherId'";
            $result = DB::exec($sql);
        }
        $start = sprintf("%02d:%02d:00", $startTime / 60, $startTime % 60);
        $end = sprintf("%02d:%02d:00", $endTime / 60, $endTime % 60);
        $sql = "INSERT INTO time_slots (start_time, end_time, teacher, `date`) VALUES ('$start', '$end','$teacherId', '$date')";
    ...
    public static function getAll()
    {
        if (!self::$timeSlots)
        {
            ...
            $sql = "SELECT id, start_time, end_time, date FROM time_slots WHERE `teacher`='ALL' ORDER BY date, start_time ASC";
	...
	public static function getTimeSlotsForTeacher($teacherId)
    {
        ...
        {
            self::$timeSlots = array(); 
            $sql = "SELECT id, start_time, end_time, date FROM time_slots WHERE `teacher`='$teacherId' ORDER BY date, start_time ASC";
            foreach (DB::query($sql) as $row)
            {
                $id = $row["id"];
                $values = explode(":", $row["start_time"]);
